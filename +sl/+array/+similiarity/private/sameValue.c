...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[])
{
    ...
    if (nrhs == 2){
        if (!mxIsClass(prhs[1],"double")){
            mexErrMsgIdAndTxt("SL:sameValue:call_error","The 2nd input must be a double");
        }
        MAX_DIFF = mxGetScalar(prhs[1]);
    }else if (nrhs != 1){
        mexErrMsgIdAndTxt("SL:sameValue:call_error","Invalid # of inputs, 1 or 2 expected");
    }
    
    if (!mxIsClass(prhs[0],"double")){
        mexErrMsgIdAndTxt("SL:sameValue:call_error","The input array must be of type double");
    }    
    
    if (!(nlhs == 1)){
        mexErrMsgIdAndTxt("SL:sameValue:n_inputs","Invalid # of outputs, 1 expected");
    }
    ...
    *(p_start+n_samples_data-1) = mxGetNaN();
    ...
    while (fabs(last_sample - current_sample) < MAX_DIFF){
...
//     }
    ...
    *(p_start+n_samples_data-1) = end_array_value;
    ...
    if (data == p_start+n_samples_data-1){
        current_sample = end_array_value;
        *pl = (fabs(last_sample - current_sample) < MAX_DIFF);
    }else{
        *pl = false;
    }
