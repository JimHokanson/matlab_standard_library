...
function s = getFigurePosition(s)
    ...
    s.position(1) = p(1) + DX;
    %In Java top is 0, In matlab top is height value
    s.position(2) = screen_size(4) - p(2) - p(4) + DY;
    s.position(3) = p(3) - 2*DX;
    s.position(4) = p(4) - 2*DY;
...
function [paramsStruct, msgStr] = convertPos(paramsStruct)
    ...
    %--------------------------------------------
    %1) Figure
    %2) Figure but no menu or toolbar
    
    s = paramsStruct;
    msgStr = '';
    
%     if isa(s.handle,'matlab.ui.Figure')
%         s = getFigurePosition(s);
%     else
...
function addToolbarButton(paramsStruct)
    ...
    if isempty(hButton)
        ...
        hButton = uipushtool(hToolbar, 'CData',cdata, 'Tag','ScreenCaptureButton', 'TooltipString',tooltip, 'ClickedCallback',['sl.os.screencapture(''' paramsStruct.target ''')']);  %#ok unused
    else
        % Otherwise, simply update the existing button
        set(hButton, 'CData',cdata, 'Tag','ScreenCaptureButton', 'TooltipString',tooltip, 'ClickedCallback',['sl.os.screencapture(''' paramsStruct.target ''')']);
    end
