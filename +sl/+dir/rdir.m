...
elseif strcmp(wildpath,'**')
  ...
  D = sl.dir.rdir([prepath postpath(2:end)]);
  ...
  Dt = dir(''); 
  for ii = 1:length(tmp),
    Dt = [Dt; sl.dir.rdir([prepath tmp(ii).name filesep wildpath postpath])]; %#ok<AGROW>
  end
  D = [D; Dt];
  ...
  D = dir(''); 
  for ii = 1:length(tmp),
    D = [D; sl.dir.rdir([prepath tmp(ii).name postpath])]; %#ok<AGROW>
  end
