...
if	~par.flg
    ...
    %Display starting to parse file
...
if	nargout
    ...
    pp.hdr = par.hdr;
    pp.res = par.res;
    if	~par.opt.dflg && isfield(pp,'par')
        pp = rmfield(pp,'par');
    else
        pp.par = par;
    end
...
%start engine
p.fnam = ''; %input filename
p.pnam = ''; %filename if extension has .p extension
p.wnam = ''; %which of filename
p.dnam = ''; %dir(wnam)
...
%         M     P     O
p.mp   = [true,true,false];
%
%FARG_ini_par       - p.mp(2)
%See FARG_get_class - p.mp(3)
...
%This is not stable, I think it might be better to resolve this
%by errors messages
%A different string is used in 2013a
par.lexerr = '<LEX_ERR>'; %String that indicates an error
%from the mlint output
par.rexcmp = '(?<='').*(?='')|(?<=(\s))\d+(?=(\.)$)';
...
if	isempty(fext) || ftype ~= par.pext{3,2}
...
if	isempty(par.wnam) || wtype ~= par.pext{3,2}
    flg = true;
    par.mp(1) = false;
    if	par.opt.sflg
...
if	par.mp(2) && ~par.pext{ftype+1,3}
    par.mp = [false,false];
end
...
%
%   Aliased as : par.fh
%   Called in  : FARG_get_file
%
%   NOTE: It would be really easy to remove this file ...
%
%   OUTPUTS
%   -----------------------------------------------------------------------
%   s  :
%       mode 0      - s is a string
%       mode 1 or 2 - s is a cellstr split on \n
%   nl : # of newlines, 1
%
%   INPUTS
%   -----------------------------------------------------------------------
%   fnam :
%       mode 0 or 1 - fnam is a file
%       mode 2      - fnam is the string to parse
%
%
%   NOTE:
...
%   par.lopt - {'-m3' '-lex'}
%
%   par.wnam - full path to file ("which() name")
%
%   Outputs po

% content
[par.file,par.nlen] = FARG_read(par.wnam,0);

% calls
par.call = mlintmex(par.wnam,'-m3','-calls');
par.call = FARG_read(par.call,2);

% tokens
par.lex  = mlintmex(par.wnam,'-m3','-lex');
par.mlex = FARG_read(par.lex,2);
...
% - empty lines
%par.wspace = [' ',sprintf('\t')];
%ERROR: this is no longer valid
ix = ismember(par.lex,par.wspace);


par.lex(ix) = '';

%line #, column start, lengths
par.lex     = textscan(par.lex,'%d/%d(%d):%[^:]:%s');

%ltok now contains the token type and string identifier
par.ltok    = [par.lex{:,4},par.lex{:,5}];

par.lex     = cat(2,par.lex{1:3});
par.nlex    = size(par.ltok,1);

%for each line, count how many times the length of the lex is 1
%like for ; = [] etc
...
%
%   This function makes a call to mlint
%
%   Called from:
%   FARG_get_file
%
...
if	~isempty(err)
    if	~par.opt.line
        fmt = par.fmtnoop;
        fnc = @(x,y,z) sprintf(['%s %5d>',fmt,': %s'],par.bol,x(1),y(1),z(1,:));
    else
        fmt=par.fmtopen;
        fnc=@(x,y,z) sprintf(['%s %5d>',fmt,': %s'],par.bol,x(1),par.wnam,y(1),y(1),z(1,:));
    end
...
if	any(lerr)
    ...
    par.txt = [par.txt; 'DONE'; {sprintf('%s LEX errors%6d',par.bol,nx) 'LINE'}];
    for	i=1:nx
        nl = par.lex(ix(i),:);
        if	par.opt.line
        ...
        par.txt = [par.txt; {sprintf('%s line  %s:   %-1d = <%s>\n',par.bol,el,nl(2),to)}; par.lint.err];
    end
    par.txt(4,1)={sprintf('%s %s\n',par.bol,repmat('-',1,size(char(par.txt(1:3)),2)-3))};
end
end
%-------------------------------------------------------------------------------
...
%This code determines if the function is a class or not
[p,par] = FARG_get_class(p,par,1);

ic = find(~cellfun(@isempty,par.stmpl(:,end))).';
%par.stmpl entries that use regular expression
%M,S,N,A,U
%main, sub, nested, anon, ML stock

for	i=ic
    fn     = par.stmpl{i,1};  %Type, M,S,N,A,U
    v.(fn) = [];	 %#ok<STRNU>
    
    %mlintmex -calls as cell array for each line
    ix = ~cellfun('isempty',regexp(par.call,['^',fn],'match'));
%     'M0 153 17 farg'
%     'E0 199 3 farg'
%     'U1 159 6 nargin'
%     'U1 160 5 nargout'
%     'U1 161 10 FARG_ini_par'
%     'U1 161 34 mfilename'
    
    
    %i.e. ix gets everything that starts with 
    
    if	any(ix)
        
        %Call the regular expression for end words/numbers on the matching
        %lines
        
        %par.rexmod = '(\w+$)|(\d+$)';
        %@(x)regexp(x,par.rexmod,'match')
        ...
        p.(fn).fn = [vtmp{:}].';
        p.(fn).nx = p.n(i);
        p.(fn).bx = [bx{:}]; %beginning
        p.(fn).ex = [ex{:}]; %end
        p.(fn).lx = cellfun(@numel,p.(fn).fn); %length
    end
end
...
%If class (classdef) and more than one M (constructor?) present ...
if	par.mp(3) && p.M.nx > 1
    
    par.lint.ferr = true;
    par.flg       = true;
    par.opt.sflg  = true;
    
    par.txt      = [par.txt 'DONE' {sprintf('%s FATAL ERROR',par.bol) 'LINE'} par.lint.err];
    par.txt(4,1) = {sprintf('%s %s\n',par.bol,repmat('-',1,size(char(par.txt(1:3)),2)-3))};
end
end
%-------------------------------------------------------------------------------
...
switch	mode
    ...
    case	1
        ...
        if	any(ich)
            %I think this is meant to find the name of the class ...
            ic  = ich;
        ...
        % - adjust complexity
    case	2
        if	par.mp(3) %This indicates that code is a class, see a few lines up
            fn=cell(par.nfun,1);
            for	i=1:3
                ix=p.ixm(:,2)==i;
                switch	i
            ...
            tcyc          = [repmat({'c'},par.nfun-numel(par.lint.ncyc),1) cyc];
            ncyc          = [repmat(-1,par.nfun-numel(par.lint.ncyc),1) par.lint.ncyc];
...
if par.nfun
    ...
    if par.mfun
        ...
        for	i = 1:par.mfun
            %Grab everything between function definition 
            %and EOL
            ...
            %par.lex
            %end line, start line, column start of first line
            ixe(i) = par.lex(ixb(i)+ixl(i),1);
            ixb(i) = par.lex(ixb(i),1);
            ixc(i) = par.lex(ixb(i),2);
            
            %?? why take the minimum here? what is ixm?
            p.ixm(i,:) = [ixb(i) min([i,2]) ixc(i)];
            
            % - N: nested
            if p.N.nx && numel(nex)
                if any(ixe(i) <= nex(1)) && any(ixe(i) >= nix(1))
                    p.ixm(i,2) = 3;
                    nex(1)     = [];
                    nix(1)     = [];
                end
...
switch	mode
    case	1
        par.txt(1,1)= {sprintf('%s parsing...          %s',par.bol,par.wnam)};
...
%FARG_sdisp Display text if par.opt.sflg
