%Same as fdep but I've reintroduced outside dependencies from
%their original sources (comments back in place). I'm also commenting the
%code to track what it is doing.
%
%Outside libraries
...
%Improvements:
%--------------------------------------------------------------------------
%1) Need a function which can resolve toolbox functions when the toolbox
%   itself is not installed
%
%   relevant example - bwconncomp - this is in the image toolbox
%
%2) Packaged functions are not handled properly   

%   sl.dep.fdep2('segWorm')

function po = fdep2(varargin)

magic = 'FDEP'; %Indeed
ver   = '21-Jun-2010 02:16:53'; %Wait, what time was it?
dopt  = {'-toponly'; '-quiet'};
mopt  = {'-m3'; '-calls' }; %mlintmex calls
%
...
%par ? - empty if error or if no inputs
if	isempty(par)
    return;
end
...
%   dopt  : 
%
%   varargin{1} - file_path or file_name 
...
if exist(fnam,'file')
    ...
    file_path = which(fnam);
    if isempty(file_path)
       error('Unable to resolve file_path') 
    end
end
fnam = file_path; %Haven't finished editing code below which uses fnam ...
...
% % % 
% % % ix = strcmp(arot,frot) & strcmp(aext,fext);
% % % 
% % % if ~any(ix)
...
% output structure
p.afile = file_path; %afnam
p.file  = file_path; %fnam

% user defined functions
p.module = {frot};
p.fun    = {fnam};
p.froot  = file_path; %{[fpat,filesep,frot]};
p.sub    = par.stmpla;
...
%
%   fnam: filename? full path to file?
%
%   Recursive call ...

par.x = par.x+1; %loop counter
par.c = par.c+1; %depth
...
%
%
%   dtmp - trace list - what is this?
%   dmlf - part of trace list that is in [matlabroot filesep 'toolbox']
%   dmod - built-in list
%   dmcl - ML classes
%   cocl - other classes
%
...
[dtmp,dmod,dmcl,docx,docx,docx,docx,docl] = depfun(fnam,'-toponly','-quiet');
