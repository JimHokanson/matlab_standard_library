classdef bspline < handle
    %
    %   Class:
    %   sci.regression.bspline
    %
    %   JAH: This was put together in a hurry and fails to expose
    %   many of the options in the real function ...
    ...
    methods
        function obj = bspline(x,y,p_or_breaks,varargin)
            %
            %   obj = sci.regression.bspline(x,y,p,varargin)
            %   
            %   obj = sci.regression.bspline(x,y,breaks,varargin)
            
            obj.x = x;
            obj.y = y;
            obj.pp = splinefit(x,y,p_or_breaks,varargin{:});
            obj.residuals = y - obj.fit(x);
            obj.sum_residuals = sum(abs(obj.residuals));
        end
        function y_fit = fit(obj,x_data)
            y_fit = ppval(obj.pp,x_data);
        end
        function plot(obj)
            yhat = ppval(obj.pp,obj.x);
            plot(obj.x,obj.y,'o')
            hold on
            plot(obj.x,yhat);
            hold off
